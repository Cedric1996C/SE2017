...
protocol personalSubTitleViewDelegate: NSObjectProtocol {
    // 当前选中第index个标题的代理毁掉
    func personalSubTitleViewDidSelected(_ titleView: personalSubTitleView, atIndex: NSInteger, title: String)
}


class personalSubTitleView: UIView {
    
    // MARK:- 代理
    weak var delegate: personalSubTitleViewDelegate?
...
// MARK:- 跳转
extension personalSubTitleView {
...
extension personalSubTitleView {
    @objc func subTitleClick(_ btn: UIButton) {
        ...
        //实际上 ? 代替了responsed
        delegate?.personalSubTitleViewDidSelected(self, atIndex: subTitleBtnArray.index(of: btn)!, title: btn.titleLabel?.text ?? "")
